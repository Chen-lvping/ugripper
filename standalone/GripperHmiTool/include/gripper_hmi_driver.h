...
private:
    ...
    uint64_t allocateCommandIdLocked();
    void logIoSummaryLocked(const char *reason, uint64_t nowMs);
    ...
    std::atomic<uint64_t> txStateRequestCount_{0};
    std::atomic<uint64_t> rxFrameCount_{0};
    std::atomic<uint64_t> rxKeyReportCount_{0};
    std::atomic<uint64_t> rxBeepStateCount_{0};
    std::atomic<uint64_t> ioFailureCount_{0};
    std::atomic<uint64_t> connectSuccessCount_{0};
    std::atomic<uint64_t> reconnectSuccessCount_{0};
    std::atomic<uint64_t> exclusiveCommandCount_{0};
    std::atomic<uint64_t> exclusiveAckCount_{0};
    std::atomic<uint64_t> exclusiveTimeoutCount_{0};
    std::atomic<uint64_t> exclusiveRetryCount_{0};
    std::atomic<uint64_t> exclusiveFailureCount_{0};
    std::atomic<uint64_t> exclusiveRecoveryCount_{0};
};
