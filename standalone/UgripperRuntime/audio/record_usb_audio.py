...
from audio_common import audio_log
...
def main() -> int:
    ...
    if target.unloaded_modules:
        audio_log(
            "INFO",
            "disabled PulseAudio suspend modules before capture init: "
    ...
    if not output_path.is_file() or output_path.stat().st_size <= 44:
        audio_log("ERROR", f"capture file not written correctly: {output_path}")
        return 2
