...
int EncoderDriver::parseReceivedData(uint8_t *data, size_t size)
{
    ...
    while (rxBuffer_.size() >= ENC_FRAME_MIN_LENGTH)
    {
        ...
        const auto frame_length = ugripper::sensor::TryGetEncoderResponseFrameLength(
            std::span<const uint8_t>(rxBuffer_.data(), rxBuffer_.size()));
        ...
        const size_t frame_len = *frame_length;
        ...
        {
            if (!ugripper::sensor::VerifyCrc16Modbus(
                    std::span<const uint8_t>(rxBuffer_.data(), 3 + len),
                    rxBuffer_[3 + len],
                    rxBuffer_[4 + len]))
            {
                rxBuffer_.erase(rxBuffer_.begin());
        ...
        else if (func == ENCODER_FUNC_WR)
        {
            ...
            stream << "EncoderDriver " << name_ << " config response:";
...
{
    const uint16_t crc = ugripper::sensor::CalculateCrc16Modbus(
...
{
    return ugripper::sensor::VerifyCrc16Modbus(
        std::span<const uint8_t>(data, len), crcLow, crcHigh);
}
