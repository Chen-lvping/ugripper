...
TEST(CameraCommandBuilderTest, BuildsHybridCommandWithStableFlags)
{
    ...
    EXPECT_NE(command.find("/usr/bin/ffmpeg -hide_banner -loglevel warning -nostats -y"),
              std::string::npos);
...
TEST(CameraCommandBuilderTest, BuildsMainCameraCommandWithExecEnvWrapper)
{
    auto config = MakeHybridCamera();
    config.mode = ugripper::camera::CameraRecordMode::DirectCopyH265;
    config.input_format = "h265";
    config.capture_width = config.width = 1920;
    config.capture_height = config.height = 1080;
    config.output_fps = config.fps;
    config.video_filter.clear();
    ...
    const std::string command = ugripper::camera::BuildMainCameraCommand(config, options);

    EXPECT_NE(command.find("exec env GST_DEBUG=identity:7 /usr/bin/gst-launch-1.0 -e"),
              std::string::npos);
    EXPECT_NE(command.find("v4l2src device='/dev/video-left' do-timestamp=true"),
              std::string::npos);
    EXPECT_NE(command.find("filesink location='/tmp/episode_001/left_main.mkv'"),
              std::string::npos);
}
...
TEST(CameraCommandBuilderTest, BuildsStereoSessionCommand)
{
    ...
    EXPECT_NE(command.find("ffmpeg -hide_banner -loglevel warning -nostats -y"),
              std::string::npos);
