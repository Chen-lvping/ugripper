...
3. 先给出简短计划，再执行实现；过程中持续保持任务上下文聚焦。
4. 如遇阻塞，在最终回复里明确写出 blocker 和下一步建议。
5. 如果你创建或修改了功能代码，尽量同步更新 docs 内相关说明。
6. 每完成一个阶段（如完成一轮排查、完成一版修复、完成一轮验证），主动执行进度上报脚本来刷新 dashboard。
7. 示例：bash "$REPORT_PROGRESS_SCRIPT" --hub-root "$REPO_ROOT" --task-id "$TASK_ID" --owner "$OWNER" --status running --message "阶段总结..."。若遇到 blocker，改用 --status blocked；任务收尾时在最终回复前也补一条阶段总结。
