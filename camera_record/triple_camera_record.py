...
class TripleCameraRecorder:
                ...
                if not found_fresh:
                    if last_system_time is not None:
                        ...
                        if sys_interval_us >= min_fresh_interval_us:
                            ...
                            print(f'[{cam_name}] 首帧: PTS={first_kernel_pts_us}, '
    ...
    def _show_summary(self, configs: list):
        ...
        if self.first_frame_info:
            ...
            min_pts = min(info['kernel_pts_us'] for info in self.first_frame_info.values())
            max_pts = max(info['kernel_pts_us'] for info in self.first_frame_info.values())
            
            for cam_name, info in sorted(self.first_frame_info.items()):
                offset_ms = (info['kernel_pts_us'] - min_pts) / 1000
                print(f"  {cam_name}: 偏移=+{offset_ms:.2f}ms, 丢弃={info['dropped']}帧")
        ...
        for config in configs:
            ...
            if intervals:
                ...
                print(f'  {cam_name} ({fps}fps): 平均={avg:.0f}us, '
...
示例:
  ...
  %(prog)s -d 30 -o my_data   # 录制 30 秒到 my_data 目录
    ...
    parser.add_argument('-d', '--duration', type=int, default=10,
                        help='录制时长(秒), 默认 10')
    parser.add_argument('-o', '--output', type=str, default='raw_data',
                        help='输出基础目录, 默认 raw_data')
    parser.add_argument('--device-id', type=str, default='ugripper_001',
