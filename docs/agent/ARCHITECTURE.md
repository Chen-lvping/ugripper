...
| --- | --- | --- | --- | --- |
| 当前侧相机录制 | `src/camera_recorder` | `./build/src/camera_recorder/camera_recorder --codec <h264|h265> --output-dir <episode> --only <current-side streams>` | 默认按 `DEVICE_SIDE` 仅录制本侧 `cam_main + tcam_l + tcam_r`。运行前由 `run_record.sh` 创建兼容旧校验链路的软链接：`cam.mkv`、`tact_left.mkv`、`tact_right.mkv`，并补一个占位 `info.json`。 | `${side}_cam_main.mkv`, `${side}_tcam_l.mkv`, `${side}_tcam_r.mkv`, 兼容软链接 |
