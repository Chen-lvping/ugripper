...
- 当前运行时轻量阈值口径：
  ...
  - residual mask 过滤为：`3x3` 内异常像素数至少 `3`，且 `8` 连通域面积至少 `16 px`
  - 该口径用于过滤轻微灰度波动和很小的孤立/半孤立连通区域，避免正常触觉图像的边缘像素漂移连续触发 damaged 软告警。
