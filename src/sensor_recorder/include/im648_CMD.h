...
// =================================移植接口======================================
     ...
     */
    extern U8 im648_Cmd_GetPkt(U8 byte);

    // 当接收到有效数据包，会回调进入到 im648_Cmd_RxUnpack(U8 *buf, U8 DLen) 函数里，用户在该函数里处理数据即可

// ================================模块的操作指令=================================
    extern U8 im648_targetDeviceAddress; // 通信地址，设为0-254指定则设备地址，设为255则不指定设备(即广播), 当需要使用485总线形式通信时通过该参数选中要操作的设备，若仅仅是串口1对1通信设为广播地址255即可
    extern void im648_Cmd_02(void);// 睡眠传感器
    extern void im648_Cmd_03(void);// 唤醒传感器
    extern void im648_Cmd_18(void);// 关闭数据主动上报
    extern void im648_Cmd_19(void);// 开启数据主动上报
    extern void im648_Cmd_11(void);// 获取1次订阅的功能数据
    extern void im648_Cmd_10(void);// 获取设备属性和状态
    /**
     * 设置设备参数
     ...
     */
    extern void im648_Cmd_12(U8 accStill, U8 stillToZero, U8 moveToZero,  U8 isCompassOn, U8 barometerFilter, U8 reportHz, U8 gyroFilter, U8 accFilter, U8 compassFilter, U16 Cmd_ReportTag);
    extern void im648_Cmd_13(void);// 惯导三维空间位置清零
    extern void im648_Cmd_16(void);// 计步数清零
    extern void im648_Cmd_14(void);// 恢复出厂校准参数
    extern void im648_Cmd_15(void);// 保存当前校准参数为出厂校准参数
    extern void im648_Cmd_07(void);// 加速计简易校准 模块静止在水平面时，发送该指令并收到回复后等待5秒即可
    /**
     * 加速计高精度校准
     ...
     *                 值255 表示要采集最后1个数据并结束
     */
    extern void im648_Cmd_17(U8 flag);

    extern void im648_Cmd_32(void);// 开始磁力计校准
    extern void im648_Cmd_04(void);// 结束磁力计校准
    extern void im648_Cmd_05(void);// z轴角归零
    extern void im648_Cmd_06(void);// xyz世界坐标系清零
    extern void im648_Cmd_08(void);// 恢复默认的自身坐标系Z轴指向及恢复默认的世界坐标系
    /**
     * 设置PCB安装方向矩阵
     * @param accMatrix 加速计方向矩阵
     * @param comMatrix 磁力计方向矩阵
     */
    extern void im648_Cmd_20(S8 *accMatrix, S8 *comMatrix);
    extern void im648_Cmd_21(void);// 读取PCB安装方向矩阵
    /**
     * 设置蓝牙广播名称
     *
     * @param bleName 蓝牙名称(最多支持15个字符长度,不支持中文)
     */
    extern void im648_Cmd_22(const char *bleName);
    extern void im648_Cmd_23(void);// 读取蓝牙广播名称
    /**
     * 设置关机电压和充电参数
     ...
     */
    extern void im648_Cmd_24(U8 PowerDownVoltageFlag, U8 charge_full_mV, U8 charge_full_mA, U8 charge_mA);
    extern void im648_Cmd_25(void);// 读取 关机电压和充电参数
    extern void im648_Cmd_26(void);// 断开蓝牙连接
    /**
     * 设置用户的GPIO引脚
     *
     * @param M 0=浮空输入, 1=上拉输入, 2=下拉输入, 3=输出0, 4=输出1
     */
    extern void im648_Cmd_27(U8 M);
    extern void im648_Cmd_2A(void);// 设备重启
    extern void im648_Cmd_2B(void);// 设备关机
    /**
     * 设置 空闲关机时长
     *
     * @param idleToPowerOffTime 当串口没有通信且蓝牙在广播中，连续计时达到这么多个10分钟则关机  0=不关机
     */
    extern void im648_Cmd_2C(U8 idleToPowerOffTime);
    extern void im648_Cmd_2D(void);// 读取 空闲关机时长
    /**
     * 设置 禁止蓝牙方式更改名称和充电参数 标识
     *
     * @param DisableBleSetNameAndCahrge 1=禁止通过蓝牙更改名称及充电参数 0=允许(默认) 可能客户的产品不想让别人用蓝牙随便改，设为1即可
     */
    extern void im648_Cmd_2E(U8 DisableBleSetNameAndCahrge);
    extern void im648_Cmd_2F(void);// 读取 禁止蓝牙方式更改名称和充电参数 标识
    /**
     * 设置 串口通信地址
     *
     * @param address 设备地址只能设置为0-254
     */
    extern void im648_Cmd_30(U8 address);
    extern void im648_Cmd_31(void);// 读取 串口通信地址
    /**
     * 设置 加速计和陀螺仪量程
     *
     ...
     */
    extern void im648_Cmd_33(U8 AccRange, U8 GyroRange);
    extern void im648_Cmd_34(void);// 读取 加速计和陀螺仪量程
    /**
      * 设置 陀螺仪自动校正标识
     *
     * @param GyroAutoFlag  1=陀螺仪自动校正灵敏度开  0=关
     */
    extern void im648_Cmd_35(U8 GyroAutoFlag);
    extern void im648_Cmd_36(void);// 读取 加速计和陀螺仪量程
    /**
      * 设置 静止节能模式的触发时长
     *
     * @param EcoTime_10s 该值大于0，则开启自动节能模式(即传感器睡眠后不主动上报，或静止EcoTime_10s个10秒自动进入运动监测模式且暂停主动上报)  0=不启用自动节能
     */
    extern void im648_Cmd_37(U8 EcoTime_10s);
    extern void im648_Cmd_38(void);// 读取 静止节能模式的触发时长
    /**
       * 设置 当前高度为指定值
     *
     * @param val 要设置的高度值 单位为mm
     */
    extern void im648_Cmd_42(S32 val);
    /**
       * 设置 自动补偿高度标识
     *
     * @param OnOff 0=关闭 1=开启
     */
    extern void im648_Cmd_43(U8 OnOff);
    // 读取 自动补偿高度标识
    extern void im648_Cmd_44(void);
    /**
       * 设置 串口波特率
     *
     * @param BaudRate 目标波特率 0=9600 1=115200 2=230400 3=460800
     */
    extern void im648_Cmd_47(U8 BaudRate);
    // 读取 串口波特率
    extern void im648_Cmd_48(void);
    // 闪烁几下led指示灯
    extern void im648_Cmd_49(void);
    /**
       * 数据透传
     *
     * @param TxBuf 要透传的数据
     ...
     */
    extern void im648_Cmd_50(U8 *TxBuf, U8 TxLen);
    /**
       * 设置 是否上传总圈数并清零
     *
     * @param isReportCycle 0=传输欧拉角数据, 1=用总圈数代替欧拉角传输 并清零圈数
     */
    extern void im648_Cmd_51(U8 isReportCycle);

    // 串口写入函数接口，需要外部实现
    extern int im648_UART_Write(const U8 *buf, int Len);

    // Note: g_im648_data_ptr is defined in im648_driver.cpp
    // im648_CMD.cpp includes im648_driver.h to get the full IM648_Data type definition
